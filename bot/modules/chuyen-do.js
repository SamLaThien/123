...
async function chuyenNhieuDo(user, items, isChat = true) {
    ...
    for (let i = 0; i < items.length; i++) {
        ...
        /*try {
            await chuyenDo(user, item, totalCh);
            await sleep(2000);
        } catch (error) {
            console.log(error);
        }*/
    }
    const userName = await getTcvNameFromTcvId(user.id);
    chat(`${userName} - Xong [img]https://love-thiduong.xyz/images/ETcmb.gif[/img]`);
}
...
async function chuyenDo(user, item) {
    ...
    const fromName = await getTcvNameFromTcvId(user.id);
    ...
    if (body === '1') {
        ...
        //chat(body);
        chat(`Giao dịch ${cap(item.name)} bị từ chối. ${body}`);
    } else {
        // Chuyển lại do lỗi hệ thống
        console.warn(`Chuyển ${item.amount} ${item.name} gặp lỗi => chờ 2s chuyển lại`);
        await sleep(2000);
        await chuyenDo(user, item);
    } 
    //if (listItem == totalCh) {
    ...
    //} //else {
    //    chat(`${fromName} - Có thể đã xong, kiểm tra lại để tránh mất đồ /denm`);
    //}
    //chat('Done!');
    //return body;
}
