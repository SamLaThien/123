...
// import {getListAcc, activeDan, inActiveDan, getLogDan} from "../cron/luyen-dan.js";
...
export async function parseMessage(parsed) {
    ...
    if (fromTcvId != '300200' && fromTcvId != '381664') {
        return;
    }
...
export async function xuLyChat(content = '', tcvId = 0, cboxId = '', tcvName = '') {
    ...
    // if (!content.startsWith(".")) {
        ...
        if (args[0] == '.checkmem' || args[0] == '.cmem') {
        ...
        try {
            if (text.toLowerCase().startsWith('cửa hàng') && args.length === 2) {
                ...
                send = send + '[br][b].hd ban:[/b] Xem vật phẩm đang BÁN trên shop';
                send = send + '[br][b].hd mua:[/b] Xem vật phẩm shop đang cần THU MUA';
                send = send + '[br][b].mua [số lượng] [vật phẩm]:[/b] Mua vật phẩm trên shop';
                send = send + '[br][b].ban [số lượng] [vật phẩm]:[/b] Bán vật phẩm trong hành trang cho shop';
                if (ADMIN.includes(tcvId)) {
                    send = send + '[br][b].hd set mua [số lượng] [vật phẩm]:[/b] Set số lượng mua';
                    send = send + '[br][b].hd set bán [số lượng] [vật phẩm]:[/b] Set số lượng bán';
                    send = send + '[br][b].hd set giá bán [số lượng] [vật phẩm]:[/b] Set giá bán';
                    send = send + '[br][b].hd set giá mua [số lượng] [vật phẩm]:[/b] Set giá mua';
                }
                pmCbox(cboxId, send);
                return;
            }
            ...
            if (text.toLowerCase().startsWith('.hd set giá bán') && ADMIN.includes(tcvId)) {
                const price = parseInt(args[args.length - 1]);
                const itemName = content.toLowerCase().replace('.hd set giá bán ', '').replace(` ${price}`, '').trim();
            ...
            if (text.toLowerCase().startsWith('.hd set bán ') && ADMIN.includes(tcvId)) {
                const amount = parseInt(args[args.length - 1]);
                const itemName = content.toLowerCase().replace('.hd set bán  ', '').replace(` ${amount}`, '').trim();
            ...
            // Nạp để bán
            if (text.toLowerCase().startsWith('.hd set giá mua') && ADMIN.includes(tcvId)) {
                const amount = parseInt(args[args.length - 1]);
                const itemName = content.toLowerCase().replace('.hd set giá mua ', '').replace(` ${amount}`, '').trim();
                await setPrice(itemName, price, true);
                pmCbox(cboxId, 'Done!');
                return;
            }

            if (text.toLowerCase().startsWith('.hd set mua ') && ADMIN.includes(tcvId)) {
                const amount = parseInt(args[args.length - 1]);
                const itemName = content.toLowerCase().replace('.hd set mua  ', '').replace(` ${amount}`, '').trim();
                await setAmount(itemName, amount, true);
                pmCbox(cboxId, 'Done!');
                return;
            }

            // Mở shop
            if (text.toLowerCase().startsWith('.hd start') && ADMIN.includes(tcvId)) {
                await activeShop(tcvId);
                return 1;
            }

            // Đóng shop
            if (text.toLowerCase().startsWith('.hd stop') && ADMIN.includes(tcvId)) {
            ...
            if (text.toLowerCase().startsWith('.ban')) {
                ...
                //if (tcvId != '666888') return;
                const amount = parseInt(args[1]);
                const itemName = content.toLowerCase().replace(`.ban ${amount} `, '').trim();
                if (isNaN(amount)) {
                    return;
                }
                ...
                response => {
                    ...
                    if (bp === 'Tu La Ma Điện' && tuVis.includes(tuVi) === false) {
                        ...
                        if (xx == 1) {
                            ...
                            chat(`${name} vừa hái được cục /gach này`);
                        } else {
                            haiLoc(tcvId);
                        }
                    }
        ...
        //chuyen bac
        // .c 300200 1234 bac
        if (args[0] == '.c' && (args[3] == 'bạc' || args[3] == 'bac' || args[3] == 'b') && Number.isInteger(args[2])) {
            ...
            try {
                ...
                for (let i = 0; i < keysChuyenBac.length; i++) {
                    ...
                    if (parseInt(expire) > 2 && fromId == tcvId) {
        ...
        */
        ...
        // if (msg.toLowerCase().includes('hi tini') || msg.toLowerCase().includes('tini ơi') || msg.toLowerCase().startsWith('tini')) {
        //     const basic = await getUserInfo(tcvId);
        ...
        /*if (msg.toLowerCase().startsWith('mai là ai')) {
            ...
            chat(`${toName} là trùm Yasuo của Tu La Ma Điện /sn`);
        }*/
        ...
        //check bac
        if (text.toLowerCase().startsWith('.cb')) {
        ...
        //check bảo khố
        if (text.toLowerCase().startsWith('.cbk')) {
            checkBaoKho(msg.split(".cbk ")[1].trim().replace(/\s\s+/g, " "), tcvName).then(() => {
            });
            return;
        }
        ...
        //duyệt all
        if (msg.toLowerCase().startsWith('.duyetall')) {
           if ((DUYET_IDS.includes(tcvId) || ADMIN.includes(tcvId))) {
        ...
        //duyet mem
        if (msg.toLowerCase().startsWith('.duyet') && args.length === 2) {
           if ((DUYET_IDS.includes(tcvId) || ADMIN.includes(tcvId))) {
        ...
        //duyệt id
        if (msg.toLowerCase().startsWith('.duyetid') && args.length === 2) {
           if ((DUYET_IDS.includes(tcvId) || ADMIN.includes(tcvId))) {
        ...
        if (text.toLowerCase().startsWith('.setch')) {
            const itemName = text.replace('.setch ', '').trim();
